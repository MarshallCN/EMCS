...
?>
	<div class='form-group'>
		...
		<div class='col-sm-2' id='timecond_ordsta' >
			<label>Expired Only <input type='checkbox' name='expired'/></label> 
		</div>
		<div class='col-sm-2'>
...
/**get the week number, change it on the input form, and change sql condition*/		
	...
	if(isset($_POST['expired'])){
		echo "<script>document.getElementsByName('expired')[0].checked = true</script>";
		if($timestamp == 'all'){
			$condition = 'WHERE status != 4';
		}else{
			$condition .= ' AND status != 4';
		}
		$expiredAdj = '<mark>Expired</mark>';
	}else{
		$expiredAdj = '';
	}
?>
